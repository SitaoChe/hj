    ...
    <div class="breadcrumb-holder">
        <div class="container-fluid">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active">选手表</li>
            </ul>
        </div>
    </div>
    <section>
                ...
                <div class="col-lg-auto">
                    <div class="card">
                        <div class="card-header">
                            <span><strong>选手表</strong></span>
                            <a href="bmdXsbToAdd.do" class="btn btn-outline-success btn-sm">添加选手</a>
                            <!--form -->
                            <!--form -->
                        </div>
                        <div class="card-body">
...
<%--                                        <td>${user.userPwd}</td>--%>
                                        ...
                                        <td>
<%--                                            <a href="bmdXsbToRace.do?userid=${user.userId}" class="btn btn-outline-success btn-sm">查看比赛信息</a>--%>
                                            <a href="bmdXsbToEdit.do?userid=${user.userId}" class="btn btn-outline-success btn-sm">编辑</a>
                                            <a href="bmdXsbRemove.do?userid=${user.userId}" class="btn btn-outline-danger btn-sm">删除</a>
                                        </td>
                                    </tr>
                                    </c:forEach>
                                    </tbody>
                                ...
                                <ul class="pagination">
                                    <c:if test="${pageInfo.hasPreviousPage}"><li class="page-item">
                                        <a class="page-link" href="bmdXsb.do?pageNum=1">首页</a></li>
                                    </c:if>
                                    <c:if test="${pageInfo.hasPreviousPage}"><li class="page-item">
                                        <a class="page-link" href="bmdXsb.do?pageNum=${pageInfo.prePage}">上一页</a></li>
                                    </c:if>
                                    <c:forEach items="${pageInfo.navigatepageNums}" var="items">
                                        <c:choose>
                                            <c:when test="${items == pageInfo.pageNum}">
                                                <li class="page-item"><a style="color:red" class="page-link" href="bmdXsb.do?pageNum=${items}">${items}</a></li>
                                            </c:when>
                                            <c:otherwise>
                                                <li class="page-item"><a class="page-link" href="bmdXsb.do?pageNum=${items}">${items}</a></li>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                    <c:if test="${pageInfo.hasNextPage}"><li class="page-item">
                                        <a class="page-link" href="bmdXsb.do?pageNum=${pageInfo.nextPage}">下一页</a></li>
                                    </c:if>
                                    <c:if test="${pageInfo.hasNextPage}"><li class="page-item" >
                                        <a class="page-link" href="bmdXsb.do?pageNum=${pageInfo.pages}">尾页</a></li>
                                    </c:if>
                                </ul>
                            </div>
                        </div>
