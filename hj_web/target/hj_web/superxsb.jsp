...
<%--                                            <td>${user.userPwd}</td>--%>
                                            ...
                                            <td>
                                                <a href="#" disabled="true" class="btn btn-outline-success btn-sm">查看</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                                ...
                                <ul class="pagination">
                                    <c:if test="${pageInfo.hasPreviousPage}"><li class="page-item">
                                        <a class="page-link" href="superXsb.do?pageNum=1">首页</a></li>
                                    </c:if>
                                    <c:if test="${pageInfo.hasPreviousPage}"><li class="page-item">
                                        <a class="page-link" href="superXsb.do?pageNum=${pageInfo.prePage}">上一页</a></li>
                                    </c:if>
                                    <c:forEach items="${pageInfo.navigatepageNums}" var="items">
                                        <c:choose>
                                        <c:when test="${items == pageInfo.pageNum}">
                                            <li class="page-item"><a style="color:red" class="page-link" href="superXsb.do?pageNum=${items}">${items}</a></li>
                                        </c:when>
                                        <c:otherwise>
                                            <li class="page-item"><a class="page-link" href="superXsb.do?pageNum=${items}">${items}</a></li>
                                        </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                    <c:if test="${pageInfo.hasNextPage}"><li class="page-item">
                                        <a class="page-link" href="superXsb.do?pageNum=${pageInfo.nextPage}">下一页</a></li>
                                    </c:if>
                                    <c:if test="${pageInfo.hasNextPage}"><li class="page-item" >
                                        <a class="page-link" href="superXsb.do?pageNum=${pageInfo.pages}">尾页</a></li>
                                    </c:if>
                                </ul>
                            </div>
                        </div>
